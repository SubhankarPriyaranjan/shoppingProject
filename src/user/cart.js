...
const Mycart = () => {
  ...
  const changeQty = async (product, action) => {
    if (action === "A") product["qty"] = product.qty + 1;
    if (action === "B") product["qty"] = product.qty - 1;
  ...
  const save = () => {
    ...
    // /^ -represent Stating the expression
    // $/ - represent End of Expression
    // [0]-we can start a number may be zero or may not be
    // [6789]- number should start from these number
              ...
              {allproduct.map((product, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td>
                      {" "}
                      <img src={product.photo} height="40" width="50" />{" "}
                    </td>
                    <td className="text-center">
                      <i
                        className="fa fa-trash fa-2x text-danger"
                        onClick={delItem.bind(this, product.id, product.name)}
                      ></i>
                    </td>
                  </tr>
                );
